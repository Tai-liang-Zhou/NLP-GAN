...
class Generated_sample(object):
    ...
    def generator_sample_example(self, positive_dir, negative_dir, num_batch):
        ...
        for i in range(num_batch):
            ...
            for i in range(FLAGS.batch_size):
                ...
                decoded_words_all, _ = re.subn(r"(\. ){2,}", ". ", decoded_words_all)
                self.write_negtive_temp_to_json(batch.original_review_inputs[i], original_review, decoded_words_all, counter)
    ...
    def write_negtive_temp_to_json(self, inputs, positive, negative, counter):
        ...
        dict = {"inputs":str(inputs),"example": str(negative),"label": str(0)}
        string_ = json.dumps(dict, ensure_ascii=False)

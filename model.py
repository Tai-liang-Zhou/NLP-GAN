...
class Generator(object):
    ...
    def _add_decoder(self, loop_function, loop_function_max, loop_given_function, input, attention_state):
        ...
        cell = tf.nn.rnn_cell.LSTMCell(hps.hidden_dim, initializer=tf.random_uniform_initializer(-0.1, 0.1, seed=123),state_is_tuple=True)
        ...
        with tf.variable_scope(tf.get_variable_scope(), reuse = True):
            ...
            decoder_outputs_max_generator,_ = tf.contrib.legacy_seq2seq.attention_decoder(
                ...
                attention_state, 
                cell, 
                loop_function = loop_function_max
            )
        ...
        with tf.variable_scope('seq2seq'):
            ...
            with tf.variable_scope('embedding'):
                ...
                if FLAGS.run_method == 'auto-encoder':
                    ...
                    encoder_outputs_word = tf.reshape(tf.tile(tf.expand_dims(encoder_outputs_word, axis = 1), [1, hps.max_dec_sen_num, 1, 1]),[hps.batch_size* hps.max_dec_sen_num, -1, hps.hidden_dim*2])
                    self.encoder_outputs_word = encoder_outputs_word
                    sentence_level_cell = tf.nn.rnn_cell.LSTMCell(hps.hidden_dim, initializer = tf.random_uniform_initializer(-0.1, 0.1, seed=123), state_is_tuple = True)
            ...
            with tf.variable_scope('output_projection'):
                w = tf.get_variable('w', [hps.hidden_dim, vsize], dtype=tf.float32, initializer=tf.truncated_normal_initializer(stddev=1e-4))
                v = tf.get_variable('v', [vsize], dtype=tf.float32, initializer=tf.truncated_normal_initializer(stddev=1e-4))

            # Add the decoder.
            with tf.variable_scope('decoder'):
                loop_function, loop_function_max, loop_given_function = sample_output(embedding, (w, v))
                
            decoder_outputs_pretrain, decoder_outputs_sample_generator, decoder_outputs_max_generator, decoder_outputs_given_sample_generator = self._add_decoder(
                loop_function=loop_function, 
            ...
            # self.decoder_outputs_pretrain = tf.reshape(tf.argmax(decoder_outputs_pretrain, 1), [hps.batch_size,hps.max_dec_sen_num , hps.max_dec_steps])
            decoder_outputs_pretrain = tf.reshape(decoder_outputs_pretrain, [hps.batch_size*hps.max_dec_sen_num, hps.max_dec_steps, vsize])

            

            decoder_outputs_sample_generator = tf.reshape(decoder_outputs_sample_generator, [hps.batch_size*hps.max_dec_sen_num * hps.max_dec_steps, hps.hidden_dim])
            ...
            decoder_outputs_given_sample_generator = tf.reshape(decoder_outputs_given_sample_generator, [hps.batch_size *hps.max_dec_sen_num* hps.max_dec_steps, hps.hidden_dim])
            decoder_outputs_given_sample_generator = tf.nn.xw_plus_b(decoder_outputs_given_sample_generator, w, v)
            self._sample_given_best_output = tf.reshape(tf.argmax(decoder_outputs_given_sample_generator, 1), [hps.batch_size,hps.max_dec_sen_num, hps.max_dec_steps])

            decoder_outputs_max_generator = tf.reshape(decoder_outputs_max_generator, [hps.batch_size*hps.max_dec_sen_num * hps.max_dec_steps, hps.hidden_dim])
